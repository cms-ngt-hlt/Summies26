...
def get_leg_arrays(df_in, colnames):
    """Pull several per-event 2-vector columns out of the RDataFrame in one pass,
    flattening leg 0 + leg 1 into 1D arrays for each column."""
...
ReconstructedTau =  {idx: cfg.Filter("Matched_tau_idx[0] >= 0 && Matched_tau_idx[1] >= 0") for idx, cfg in channel.items()} 
ReconstructedJet =  {idx: cfg.Filter("Matched_jet_idx[0] >= 0 && Matched_jet_idx[1] >= 0") for idx, cfg in channel.items()} 
ReconstructedBoth = {idx: cfg.Filter("Matched_tau_idx[0] >= 0 && Matched_tau_idx[1] >= 0 && Matched_jet_idx[0] >= 0 && Matched_jet_idx[1] >= 0") for idx, cfg in channel.items()} 
...
for ch in channels:
    plot_reco_efficiency("Reco_jet_pT", ch, save_as='png', scale='lin', both=True)
    plot_reco_efficiency("Reco_jet_eta", ch, save_as='png', scale='lin', both=True)
