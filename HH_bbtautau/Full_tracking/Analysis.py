...
df = ROOT.RDataFrame("Events", "/eos/user/s/sbenabde/CERN_Summer_student/Dataframes/df_full_tracking.root")
...
streams_reco = {
    "Reco_tau_e":   channel[0].Filter("DST_PFScouting == true").Filter("Matched_tau_idx[0] >= 0 && Matched_tau_idx[1] >= 0"), #1 electron matched & 1 tau matched
...
var_configs = {
    ...
    "Gen_H_bbjets_mass":   {"branch": "Gen_H_bbjets_mass",  "xlabel": r"Gen $m_H(b \bar{b})$",  "bins": 30, "range": (10, 300)},
    "Gen_H_bb_mass":       {"branch": "Gen_H_bb_mass",      "xlabel": r"Gen $m_H(b \bar{b})$",  "bins": 50, "range": (120, 130)},
    "Gen_H_tt_mass":       {"branch": "Gen_H_tt_mass",      "xlabel": r"Gen $m_H(\tau \tau)$",  "bins": 50, "range": (120, 130)},
...
def plot_kinematic(var, ch):
    path = "/eos/user/s/sbenabde/CERN_Summer_student/HH_bbtautau/Full_tracking/plots/kinematics/"
...
def plot_efficiency(var, ch, save_as, scale):
    path = "/eos/user/s/sbenabde/CERN_Summer_student/HH_bbtautau/Full_tracking/plots/Efficiency"
    os.makedirs(path, exist_ok=True)
    ...
    ax.text(0.18, -0.53, "Full Tracking", transform=ax.transAxes, fontsize=26, ha="right", va="top", fontweight="bold")
...
def plot_reco_efficiency(var, ch, save_as, scale, No_tag = False, Tau=False, Jet=False, Both=False):
    ...
    if Tau==True:
        ...
        path = "/eos/user/s/sbenabde/CERN_Summer_student/HH_bbtautau/Full_tracking/plots/Reco_Efficiency_noBTagging"

    if Jet==True:
        arr_reco = data[f"{var}_Reco_jet_{suf}"]
        arr_reco_tagged = data[f"{var}_Reco_bjet_{suf}"]
        lb   = r"Matched+b-tagged"
        path = "/eos/user/s/sbenabde/CERN_Summer_student/HH_bbtautau/Full_tracking/plots/Reco_Efficiency_noTauTagging"
    
    if Both==True:
        arr_reco = data[f"{var}_Reco_both_{suf}"]
        arr_reco_tagged = data[f"{var}_Reco_both_tagged_{suf}"]
        lb   = r"Matched+$\tau$&b-tagged"
        path = "/eos/user/s/sbenabde/CERN_Summer_student/HH_bbtautau/Full_tracking/plots/Reco_Efficiency_withTagging"
   
    if No_tag==True: 
        arr_reco        = data[f"{var}_NGT_{suf}"]
        arr_reco_tagged = data[f"{var}_NGT_{suf}"]
        lb   = r"Matched"
        path = "/eos/user/s/sbenabde/CERN_Summer_student/HH_bbtautau/Full_tracking/plots/Reco_Efficiency_noTagging"
